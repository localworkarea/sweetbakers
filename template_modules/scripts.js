...
async function insertModule(html) {
			...
			if (curentModulePlugins) {
				curentModulePlugins.forEach(curentModulePlugin => {
					...
					if (pluginRegex.test(html)) {
						modules.add(curentModulePlugin)
					}
				})
			}
		}
	...
	const bootstrapImports = Array.from(modules)
		.map(modulePath => `import "${modulePath}";`)
		.join('\n')
	const bootstrapModule = `<script type="module">\n${bootstrapImports}\n</script>`
	return html.replace('</head>', `${bootstrapModule}</head>`)
}
