import { b as bodyLock, a as bodyUnlock, c as bodyLockStatus, d as bodyLockToggle, s as slideUp, e as slideToggle, g as gotoBlock, u as uniqArray } from "./common.min.js";
(function polyfill() {
...
function initSliders() {
  ...
  if (document.querySelector(".home-brands__slider")) {
    let updateExtraClasses = function(swiper) {
      const slides = Array.from(swiper.slides);
      slides.forEach((slide2) => {
        slide2.classList.remove("swiper-slide-prev-prev", "swiper-slide-next-next");
      });
      const prevSlide = slides.find((slide2) => slide2.classList.contains("swiper-slide-prev"));
      const nextSlide = slides.find((slide2) => slide2.classList.contains("swiper-slide-next"));
      const canAddExtraClass = (slide2) => {
        return slide2 && !slide2.classList.contains("swiper-slide-active") && !slide2.classList.contains("swiper-slide-prev") && !slide2.classList.contains("swiper-slide-next");
      };
    ...
    const originalSlides = Array.from(wrapperEl.children).filter(
...
if (btnMain.length > 0) {
  ...
  const media2 = window.matchMedia("(min-width: 820px)");
  btnMain.forEach((btn) => {
    ...
    check(media2);
    media2.addEventListener("change", check);
  });
}
class ScrollWatcher {
  constructor(props) {
...
function scrollToTarget(target, { updateHash = false, duration = null } = {}) {
  ...
  const finalDuration = duration ?? (distance < 300 ? 1.4 : distance < 900 ? 1.8 : 3);
